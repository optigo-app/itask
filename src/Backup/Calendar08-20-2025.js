    ...
    const calendarOptions = {
        ...
        eventReceive({ event: receivedEvent }) {
            ...
            const eventExists = existingData.some(event =>
                ...
                JSON.stringify(event.guests) === JSON.stringify(newEvent.guests) &&
                event.eventUrl === newEvent.eventUrl &&
                event.allDay === newEvent.allDay
            );
            if (eventExists) {
                console.warn("Duplicate event detected, removing from calendar UI...");
